...
class CloakRequest(BaseModel): cloaked: str

@app.post("/generate") def generate_text(req: PromptRequest): return {"output": f"Generated (NSFW):: {req.prompt}"}

@app.post("/cloak") def cloak_prompt(req: PromptRequest): encoded = base64.b64encode(req.prompt.encode()).decode() return {"cloaked": encoded}

@app.post("/decloak") def decloak_prompt(req: CloakRequest): decoded = base64.b64decode(req.cloaked).decode() return {"prompt": decoded}

@app.post("/image-prompt", operation_id="generateImagePrompt") def generate_image_prompt(req: PromptRequest): base = req.prompt
